...
{
controller_interface::CallbackReturn LidarSensorBroadcaster::on_init()
{
  try
  {
...
controller_interface::CallbackReturn LidarSensorBroadcaster::on_configure(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  params_ = param_listener_->get_params();

  lidar_sensor_ = std::make_unique<semantic_components::LidarSensor>(
    semantic_components::LidarSensor(params_.sensor_name));
  try
  {
    // register ft sensor data publisher
    sensor_state_publisher_ =
      get_node()->create_publisher<sensor_msgs::msg::LaserScan>("~/laser_scan", rclcpp::SystemDefaultsQoS());
...
controller_interface::CallbackReturn LidarSensorBroadcaster::on_activate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
...
controller_interface::CallbackReturn LidarSensorBroadcaster::on_deactivate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
